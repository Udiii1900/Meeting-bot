...
def fetch_appointments(from_ms, to_ms):
    ...
    while True:
        body = {
            "filterGroups": [{
                "filters": [
                    {"propertyName": "hs_timestamp", "operator": "GTE", "value": str(from_ms)},
                    {"propertyName": "hs_timestamp", "operator": "LT", "value": str(to_ms)}
                ]
            }],
        ...
        # Fallback falls hs_timestamp nicht filterbar ist
        if resp.status_code == 400:
            body["filterGroups"][0]["filters"] = [
                {"propertyName": "hs_start_time", "operator": "GTE", "value": str(from_ms)},
                {"propertyName": "hs_start_time", "operator": "LT", "value": str(to_ms)}
            ]
            resp = requests.post(url, headers=HEADERS, json=body)
